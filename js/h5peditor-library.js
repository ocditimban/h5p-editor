...
H5PEditor.Library = (function ($, H5P) {
   ...
   */
  function WidgetLibrary(parent, field, params, setValue) {
    var self = this;
  ...
  WidgetLibrary.prototype = Object.create(H5P.EventDispatcher.prototype);
  WidgetLibrary.prototype.constructor = H5PEditor.Library;
   ...
   */
  WidgetLibrary.prototype.appendTo = function ($wrapper) {
    var that = this;
    var html = '';
    ...
    H5PEditor.LibraryListCache.getLibraries(that.field.options, that.librariesLoaded, that);
  };
   ...
   * @param {Array} libList
   */
  WidgetLibrary.prototype.librariesLoaded = function (libList) {
    this.libraries = libList;
    var self = this;
    var options = H5PEditor.createOption('-', '-');
    for (var i = 0; i < self.libraries.length; i++) {
      ...
      if (library.uberName === self.params.library ||
        ...
        options += H5PEditor.createOption(library.uberName, library.title, library.uberName === self.params.library);
      }
    }
   ...
   */
  WidgetLibrary.prototype.loadLibrary = function (libraryName, preserveParams) {
    var that = this;
    ...
    this.$libraryWrapper.html(H5PEditor.t('core', 'loading', {':type': 'semantics'})).attr('class', 'libwrap ' + libraryName.split(' ')[0].toLowerCase().replace('.', '-') + '-editor');

    H5PEditor.loadLibrary(libraryName, function (semantics) {
      ...
      H5PEditor.processSemanticsChunk(semantics, that.params.params, that.$libraryWrapper.html(''), that);
   ...
   */
  WidgetLibrary.prototype.change = function (callback) {
   ...
   * @returns {boolean}
   */
  WidgetLibrary.prototype.validate = function () {
    if (this.params.library === undefined) {
   ...
   */
  WidgetLibrary.prototype.ready = function (ready) {
    if (this.passReadies) {
   ...
   */
  WidgetLibrary.prototype.removeChildren = function () {
    if (this.currentLibrary === '-' || this.children === undefined) {
      return;
    }

    var ancestor = H5PEditor.findAncestor(this.parent);
   ...
   * @param {Function} task
   */
  WidgetLibrary.prototype.forEachChild = function (task) {
    for (var i = 0; i < this.children.length; i++) {
   ...
   */
  WidgetLibrary.prototype.remove = function () {
    this.removeChildren();
...
H5PEditor.widgets.library = H5PEditor.Library;

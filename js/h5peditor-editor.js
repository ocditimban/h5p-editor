...
H5PEditor.Editor = (function ($) {
   ...
   */
  WidgetEditor = function (library, defaultParams, replace) {
    var self = this;
   ...
   */
  WidgetEditor.prototype.getLibrary = function () {
   ...
   */
  WidgetEditor.prototype.getParams = function () {
    if (this.selector !== undefined) {
   ...
   */
  H5PEditor.t = function (library, key, vars) {
    if (H5PEditor.language[library] === undefined) {
      return 'Missing translations for library ' + library;
    }

    var translation;
    if (library === 'core') {
      if (H5PEditor.language[library][key] === undefined) {
        return 'Missing translation for ' + key;
      }
      translation = H5PEditor.language[library][key];
    }
    else {
      if (H5PEditor.language[library].libraryStrings === undefined || H5PEditor.language[library].libraryStrings[key] === undefined) {
        return H5PEditor.t('core', 'missingTranslation', {':key': key});
      }
      translation = H5PEditor.language[library].libraryStrings[key];
    }
   ...
   */
  H5PEditor.wrap = function (prefix, content, suffix) {
    var result = '';
